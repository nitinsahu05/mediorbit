...
const loginDoctor = async (req, res) => {
  try {
    ...
    if (!user) {
      console.log(`Login attempt - Doctor not found: ${email}`);
      return res.status(401).json({ success: false, message: "Invalid credentials" });
    }

    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) {
      // Legacy support: if the stored password is plain text, allow login and auto-hash the password for future security.
  ...
  try {
    ...
    if (imageFile) {
      if (!isCloudinaryEnabled()) {
        return res.status(400).json({ success: false, message: 'File upload is disabled. Please configure Cloudinary or use image URL flow.' });
      }
